...
impl BytePacketBuffer {
    ...
    fn read(&mut self) -> Result<u8> {
        ensure!(self.pos < 512, "End of buffer");
    ...
    fn get(&mut self, pos: usize) -> Result<u8> {
        ensure!(pos < 512, "End of buffer");
        Ok(self.buf[pos])
    }

    fn get_range(&mut self, start: usize, len: usize) -> Result<&[u8]> {
        ensure!(start + len < 512, "End of buffer");
    ...
    fn read_qname(&mut self, outstr: &mut String) -> Result<()> {
        ...
        loop {
            ...
            ensure!(jumps_performed <= max_jumps, "Limit of {} jumps exceeded", max_jumps);
    ...
    fn write_qname(&mut self, qname: &str) -> Result<()> {
        for label in qname.split('.') {
            let len = label.len();
            ensure!(len <= 63, "Single label exceeds 63 characters of length");
...
async fn handle_query(socket: &UdpSocket, hickory_resolver: &TokioResolver) -> Result<()> {
    let mut req_buffer = BytePacketBuffer::new();
...
pub(crate) async fn server(resolver: TokioResolver) -> Result<()> {
    let socket = UdpSocket::bind(("127.0.0.1", 53))?;

    loop {
        match handle_query(&socket, &resolver).await {
            Ok(_) => {}
